...
def algo_moindre_cout(canvas_frame, text_area, nb_usines, nb_magasins):
    """
    ...
    def methode_moindre_cout(couts, capacites, demandes):
        ...
        couts_trie = sorted([(i, j, couts[i, j]) 
                             for i in range(len(capacites)) 
                             for j in range(len(demandes))], key=lambda x: x[2])
    ...
    allocation = methode_moindre_cout(couts, capacites.copy(), demandes.copy())
    ...
    result = "Solution de transport (Moindre Coût) :\n"
